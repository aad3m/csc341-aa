...
public class TestListEnhanced extends Assert {
	...
	public static void main(String[] args) {
		...
		*/
		...
		try {
			...
			auggies.add(s1);
			assertEquals(1,auggies.size(),"First Add (count)");
	...
	public static void testAddAll() throws Exception {
		...
		auggies = new AuggiesList(8);

		Auggie[] toAdd = {s0,s1,s2};

		auggies.addAll(toAdd);
		assertEquals(3,auggies.size(),"addAll to empty (size)");
		...
		assertEquals(6,auggies.size(),"addAll add 3 to 3 (size)");
		...
		try {
			auggies.addAll(toAdd);
				...
				assertEquals(6,auggies.size(),"attempt add over capacity");
		}

		// add to limit
		Auggie[] two = {s2,s5};
		auggies.addAll(two);
		assertEquals(8,auggies.size(),"addAll up to capacity");

	} // end testConstructorGetters

	/* __________________________________________________________________ */
	public static void testFindAuggie() throws Exception {
		System.out.println("\n\n_________________________________________");
		...
		System.out.println("--------------findAuggie(person)");
		auggies = new AuggiesList(8);
		auggies.add(grace);
		auggies.add(kate);
		auggies.add(neal);
	...
	public static void testRemoves() throws Exception {
		...
		try {
			...
			// expected behavior
			assertEquals(count,auggies.size(),"size changed but invalid index 4.");
		}
		...
		try {
			...
			// expected behavior
			assertEquals(count,auggies.size(),"size changed but invalid index -1.");
		}
		...
		try {
			...
			assertEquals(count,auggies.size(),"size changed but invalid auggie");
		}
		...
		auggies.remove(neal);
		assertEquals(3,auggies.size(),"remove middle (neal)");

		// remove last (bert) ==> {grace,kate}
		auggies.remove(bert);
		assertEquals(2,auggies.size(),"remove last (bert)");

		// remove first (grace) ==> {kate}
		auggies.remove(grace);
		assertEquals(1,auggies.size(),"remove first (grace)");

		// remove only (kate) ==> {}
		auggies.remove(kate);
		assertEquals(0,auggies.size(),"remove only student (length)");
		
		// _________________________________________________________
		System.out.println("--------------removeAll(Auggie)");
		auggies = new AuggiesList(6);
		
		// remove from empty
		count = auggies.size();
		auggies.removeAll(Role.FACULTY);
		assertEquals(0,auggies.size(),"removeAll faculty from empty");
		...
		auggies.removeAll(Role.FACULTY);
		auggies.removeAll(Role.STAFF);
		assertEquals(2,auggies.size(),"removeAll faculty (size)");
		assertEquals(auggies.get(0),kate,"removeAll (kate)");
		assertEquals(auggies.get(1),bert,"removeAll (bert)");
	} // end testRemoves
	...
	public static void testConverts() throws Exception {
		...
		try {
			...
			// expected behavior. all good.
			assertEquals(count,auggies.size(),"size changed but array too small.");
		}
		
		System.out.println("--------------Testing sublist()");
		
		auggies = new AuggiesList(10);
		auggies.add(s0);
		auggies.add(s1);
		auggies.add(s2);
		auggies.add(s3);
